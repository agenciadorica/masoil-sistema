    ...
    <div className="p-8 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="p-6">
          ...
          <p className="text-3xl font-bold mb-2">{formatCurrency(avgOrderValue)}</p>
          <div className="flex items-center gap-1 text-sm text-purple-600">
            <TrendingUp className="h-4 w-4" />
            <span>+6.5% vs mes anterior</span>
          </div>
        </Card>
        ...
        <Card className="p-6">
          ...
          <p className="text-3xl font-bold mb-2">{fulfillmentRate}%</p>
      ...
      <Card className="p-6">
        ...
        <div className="space-y-2">
          {topProducts.map((product, index) => (
            <div key={product.id} className="flex items-center gap-4 p-4 border rounded-lg hover:bg-muted/50">
              ...
              <div className="text-right">
                ...
                <p className="text-sm text-muted-foreground">{product.unitsSold} unidades vendidas</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          {salesByZone.map((zona) => (
            <Card key={zona.zona} className="p-4 border-2">
              <h4 className="font-semibold mb-3">{zona.zona}</h4>
              <div className="space-y-2">
                <div>
                  <p className="text-xs text-muted-foreground">Ventas</p>
                  <p className="text-xl font-bold">{formatCurrency(zona.ventas)}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Pedidos</p>
                  <p className="text-lg font-semibold">{zona.pedidos}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Ticket Promedio</p>
                  <p className="text-sm font-medium">{formatCurrency(zona.ventas / (zona.pedidos || 1))}</p>
                </div>
              </div>
            </Card>
          ))}
